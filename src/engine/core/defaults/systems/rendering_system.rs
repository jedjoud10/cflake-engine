...
pub fn create_system(world: &mut World) {
    ...
    rs.get_system_component_mut::<RendererS>(world)
        .quad_renderer_index = index;
    ...
    rs.system_pre_loop_event = |world, system| unsafe {
        gl::BindFramebuffer(
            gl::FRAMEBUFFER,
            system.get_system_component::<RendererS>(world).framebuffer,
        );
    ...
    rs.system_post_loop_event = |world, system| {
        ...
        shader.set_texture2d(
            ...
            system_component.color_texture.id,
            gl::TEXTURE0,
        );
        shader.set_texture2d(
            shader.get_uniform_location("normals_texture"),
            system_component.normals_texture.id,
            gl::TEXTURE1,
        );
        shader.set_texture2d(
            shader.get_uniform_location("position_texture"),
            system_component.position_texture.id,
            gl::TEXTURE2,
        );
        shader.set_scalar_3_uniform(
            shader.get_uniform_location("view_pos"),
        ...
        shader.set_int_uniform(
            ...
            system_component.debug_view as i32,
        );
