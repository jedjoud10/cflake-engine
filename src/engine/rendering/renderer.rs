...
impl Renderer {
    ...
    pub fn load_textures(
        ...
        for (i, &texture_path) in texture_paths.iter().enumerate() {
            ...
            let mut texture = Texture::new().set_mutable(false).set_idf(gl::RGBA, gl::RGBA, gl::UNSIGNED_BYTE).load_texture(texture_path, resource_manager, texture_manager).unwrap();
