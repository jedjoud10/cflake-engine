...
impl VoxelGenerator {
    ...
    pub fn generate_voxels_end(&mut self, _size: u64, _depth: u8, _position: veclib::Vector3<i64>) -> Option<Box<[Voxel]>> {
        ...
        let voxel_pixels = pipec::task_immediate(pipeline::RenderTask::TextureFillArray(self.voxel_texture, 4), "get_voxel_density").unwrap();
        let material_pixels = pipec::task_immediate(pipeline::RenderTask::TextureFillArray(self.material_texture, 2), "get_voxel_material").unwrap();
