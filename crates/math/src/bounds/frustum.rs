...
impl<T: Real> Frustum<T> {
    ...
    pub fn iter(
        &self,
    ) -> impl ExactSizeIterator<Item = &FrustumPlane<T>> {
        self.0.iter()
    }
    ...
    pub fn iter_mut(
        &mut self,
    ) -> impl ExactSizeIterator<Item = &mut FrustumPlane<T>> {
        self.0.iter_mut()
    }
}
    ...
    ($t:ty) => {
        impl Frustum<$t> {
            ...
            pub fn from_camera_matrices(
                projection: vek::Mat4<$t>,
                view: vek::Mat4<$t>,
            ) -> Self {
                let columns = (projection * view)
                    .transposed()
                    .into_col_arrays();
                let columns = columns
                    .into_iter()
                ...
                let right =
                    FrustumPlane::new(columns[3] - columns[0]);
                let top = FrustumPlane::new(columns[3] - columns[1]);
                let bottom =
                    FrustumPlane::new(columns[3] + columns[1]);
