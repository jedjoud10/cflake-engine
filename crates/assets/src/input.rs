...
pub trait AssetInput<'str, 'ctx, 'stg, A: Asset> {
    fn split(self) -> (&'str str, A::Context<'ctx>, A::Settings<'stg>);
...
{
    fn split(self) -> (&'str str, <A as Asset>::Context<'ctx>, <A as Asset>::Settings<'stg>) {
...
impl<'str, 'ctx, 'stg, A: Asset> AssetInput<'str, 'ctx, 'stg, A> for (&'str str, A::Settings<'stg>)
where
