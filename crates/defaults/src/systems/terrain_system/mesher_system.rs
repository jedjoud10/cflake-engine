...
use main::{
    ...
    rendering::{pipeline::pipec, basics::{material::Material, model::Model, renderer::RendererFlags}, object::ObjectID},
    terrain::{
    ...
    if let Ok(mut terrain) = terrain {
        ...
        for (id, components) in query.write().iter_mut() {
            if terrain.chunks_manager.current_chunk_state == ChunkGenerationState::EndVoxelDataGeneration(*id, true) {
                ...
                let chunk = components.get_component::<crate::components::Chunk>().unwrap();
...
fn create_chunk_renderer_linking_group(model_id: ObjectID<Model>, material: ObjectID<Material>) -> ComponentLinkingGroup {
    ...
    group.link(crate::components::Renderer::new(RendererFlags::DEFAULT)
        .with_model(model_id)
