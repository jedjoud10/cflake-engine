...
fn run(world: &mut World, _data: EventKey) {
    ...
    egui::Window::new("Debug Window")
        ...
        .show(gui, |ui| {
            ...
            ui.label(format!(
                "Camera Pos: '{}'",
                (data.camera_pos * 10.0).round() / 10.0
            ));
            ui.label(format!(
                "Camera Dir: '{}'",
                (data.camera_forward * 10.0).round() / 10.0
            ));
            // Timings
            ui.separator();
            ui.heading("Timings");
            ...
            if let Ok(terrain) = terrain {
                ...
                ui.label(format!(
                    "Pending Generation: '{}'",
                    terrain.chunks_manager.chunks_generating.len()
                ));
                ui.label(format!(
                    "Pending Deletion: '{}'",
                    terrain.chunks_manager.chunks_to_remove.len()
                ));
                ui.label(format!(
                    "Total Edits: '{}'",
                    terrain.editing_manager.edits.len()
                ));
            }
            // Rendering
            ui.separator();
            ...
            ui.label(format!(
                "Render Frame Time: '{:.1}'",
                debuginfo.render_frame
            ));
            ui.label(format!(
                "EoF Callbacks Execution Time: '{:.1}'",
                debuginfo.eof_callbacks_execution
            ));
            ui.label(format!(
                "Swap Buffers Time: '{:.1}'",
                debuginfo.swap_buffers
            ));
        });
}
