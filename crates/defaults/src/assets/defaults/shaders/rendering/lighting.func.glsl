...
vec3 compute_lighting(
	...
	vec3 ambient = calculate_sky_color(sky_texture, normal, sky_light_val, time_of_day);

	// Add everything
	vec3 ambient_lighting = ambient * 0.02;
	vec3 def = diffuse * 0.03 + ambient_lighting;
	vec3 color = def + (1 - in_shadow) * (diffuse * light_val);
