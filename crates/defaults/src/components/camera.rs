...
impl Camera {
    ...
    pub fn update_projection_matrix(&mut self) {
        ...
        let vertical_fov: f32 = 2.0
            * ((self.horizontal_fov.to_radians() / 2.0).tan() * (1.0 / (self.aspect_ratio))).atan();
        self.projection_matrix = veclib::Matrix4x4::<f32>::from_perspective(
            self.clip_planes.x,
            self.clip_planes.y,
            self.aspect_ratio,
            vertical_fov,
