...
float calculate_depth_difference(vec2 nf_planes, sampler2D depth_texture, vec3 uvs) {    
    ...
    if (any(lessThan(uvs, vec3(0))) || any(greaterThan(uvs, vec3(1)))) {
        return 999999.0;
    }
...
vec3 calculate_ssr(vec3 pixel_dir, vec3 position, vec2 nf_planes, vec3 normal, sampler2D depth_texture, sampler2D diffuse_texture, mat4x4 pv_matrix) {
    ...
    for(int i = 0; i < MAX_STEPS; i++) {
        ...
        if (depth_diff > 0.0 && depth_diff < 0.06) {
            ...
            for(int j = 0; j < MAX_STEPS_FINE; j++) {
                ...
                if (depth_diff_fine > 0.0 && depth_diff_fine < 0.03) {
