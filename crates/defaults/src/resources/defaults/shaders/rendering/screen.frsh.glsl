...
void main() {
	...
	float sky_gradient_x = (sin(_time * 0.5) + 1.0) / 2.0;
	...
	vec3 sky_color = texture(default_sky_gradient, vec2(sky_gradient_x, sky_uv_sampler)).xyz;
	// Add the sun
	...
	vec3 reflected_color = texture(default_sky_gradient, vec2(sky_gradient_x, reflected.y)).xyz;


	// Used for ambient lighting
	float ambient_lighting_strength = 0.01;
	float sky_light_val = dot(normal, vec3(0, 1, 0)); 
	vec3 ambient_lighting_color = texture(default_sky_gradient, vec2(sky_gradient_x, sky_light_val)).xyz;
