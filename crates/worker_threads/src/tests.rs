...
pub mod test {
    ...
    pub fn test() {
        ...
        pool.execute(numbers.iter_mut().map(|x| x as *mut i32).collect::<Vec<_>>(), &(), |_, _| {});
    }
    #[test]
    ...
    pub fn speed_test() {
        ...
        pool.execute(numbers1.iter_mut().map(|x| x as *mut i32).collect::<Vec<_>>(), &(), |a, b| { *b = expensive_calculation() });
