...
fn update_hierarchy(world: &mut World, _: &Update) {
...
// This plugin will update the scene hierarchy with the proper local offsets and rotations

pub fn plugin(registries: &mut Registries) {
    registries.update
        .insert(update_hierarchy)
        .after(post_user);
        //.before(ecs);
}
