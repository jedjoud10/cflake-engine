...
impl QueryCache {
    ...
    pub fn insert_with_capacity<'a, Layout: QueryLayout<'a>>(&mut self, capacity: usize) -> Option<()> {
        // Check first
        let mask = Layout::layout_mask().ok()?;
        if self.vecs.contains_key(&mask) { return None; }

        // Vector init
        let vec = Vec::<Layout::PtrTuple>::with_capacity(capacity);
        let boxed: Box<dyn Any> = Box::new(vec);
        self.vecs.insert(mask, boxed);
        Some(())
    }
    // Get a layout vector pointer using it's layout type
    pub fn get_mut<'a, Layout: QueryLayout<'a>>(&mut self) -> Option<&mut Vec<Layout::PtrTuple>> {
        let boxed = self.vecs.get_mut(&Layout::layout_mask().ok()?)?;
        let vec = boxed.downcast_mut::<Vec<Layout::PtrTuple>>()?;
        Some(vec)
    }
}
