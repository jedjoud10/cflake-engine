...
pub struct EventHandler<Context> {
    evn_run_system: Vec<fn(Context, ComponentQuery)>,
    evn_added_entity: Vec<fn(Context, ComponentQuery)>,
    evn_removed_entity: Vec<fn(Context, ComponentQuery)>,
}
...
impl<Context> EventHandler<Context> {
    ...
    pub fn add_added_entity_event(&mut self, event: fn(Context, ComponentQuery)) -> usize {
        self.evn_added_entity.push(event);
        self.evn_added_entity.len() - 1
    }
    // Add a "Removed Entity" event to the EventHandler
    pub fn add_removed_entity_event(&mut self, event: fn(Context, ComponentQuery)) -> usize {
