...
impl std::fmt::Debug for LinkError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LinkError::StrictLinkOutlier(name) => write!(f, "Unable to add component of type '{}' because the target archetype does not accept it", name)
        }
    }
}
