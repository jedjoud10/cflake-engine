...
impl ArchetypeStates {
    ...
    pub fn set_all_component_states(&mut self, mask: Mask, state: bool) {
        ...
        for row in collumn { write_bit_u8(row, state, shift % MINI); }
    }
    
    // Get the state of an entity
    pub fn get_entity_state(&self, bundle: usize) -> Option<EntityState> {
        // Check if the index is valid
        if bundle >= self.length { return None }
    ...
    pub fn reset_entity_states(&mut self) {
        for x in self.entities.iter_mut() { *x = 0 }
    }
    ...
    pub fn iter_entities(&self) -> EntityStatesIter {
        EntityStatesIter { states: self, chunk: None, bundle: 0  }
    }
    /*
...
impl<'a> Iterator for EntityStatesIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        // Check if the index is valid
        if self.bundle >= self.states.length { return None; }
        ...
        let chunk = self.chunk.get_or_insert_with(|| {
            *self.states.entities.get(self.bundle / HALF).unwrap()
        });
