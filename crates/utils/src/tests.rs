...
mod atomic_vec {
    ...
    fn iter() {
        ...
        let iter = vec.iter();
        let values = iter.collect::<Vec<&u32>>();
        assert_eq!(**values.get(0).unwrap(), 5);
        assert_eq!(**values.get(1).unwrap(), 1);
        assert_eq!(**values.get(2).unwrap(), 2);
        drop(values);
        ...
        assert_eq!(**values.get(1).unwrap(), 1);
        assert_eq!(**values.get(2).unwrap(), 2);
        assert_eq!(**values.get(3).unwrap(), 9);
        assert_eq!(**values.get(4).unwrap(), 50);
        */
    }
}
