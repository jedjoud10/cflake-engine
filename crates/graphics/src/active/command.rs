...
pub(crate) enum RenderCommand<'a, C: ColorLayout, DS: DepthStencilLayout> {
    ...
    // Multi draw indirect command without index buffer with count buffer
    MultiDrawIndirectCount {
        buffer: &'a DrawIndirectBuffer,
        indirect_offset: usize,
        count: &'a DrawCountIndirectBuffer,
        count_offset: usize,
        max_count: u32,
    },

    // Multi draw indirect command with the current bound index buffer with count buffer
    ...
    for render_command in render_commands {
        match render_command {
            ...
            RenderCommand::SetVertexBuffer {
                ...
                let bound = (*start, *end);
                render_pass.set_vertex_buffer(*slot, buffer.raw().slice(bound))
            }

            RenderCommand::SetIndexBuffer { buffer, start, end } => {
                let bound = (*start, *end);
                render_pass.set_index_buffer(buffer.raw().slice(bound), wgpu::IndexFormat::Uint32);
            }
            ...
            RenderCommand::SetPushConstants {
                ...
                render_pass.set_push_constants(*stages, *local_offset as u32, data);
            }
    ...
    for compute_command in compute_commands {
        match compute_command {
            ...
            ComputeCommand::SetPushConstants {
                ...
                let end = global_offset + size;
                let data = &push_constants[start..end];
                compute_pass.set_push_constants(*local_offset as u32, data);
            }
