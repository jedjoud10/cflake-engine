...
pub enum SetTextureError {
    ...
    #[error(
        "Texture is defined as storage RW/Write in the shader, but the given texture is immutable"
    )]
    MutabilityMissing,
}
...
pub enum SetBufferError {
    ...
    #[error(
        "Buffer is defined as storage RW/Write in the shader, but the given buffer is immutable"
    )]
    MutabilityMissing,
}
