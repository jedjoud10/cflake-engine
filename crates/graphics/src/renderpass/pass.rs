...
impl Drop for RenderPass {
    fn drop(&mut self) {
        unsafe {
            self.graphics
                .device()
                .destroy_render_pass(self.renderpass);
            self.graphics
                .device()
...
impl RenderPass {
    ...
    pub unsafe fn resize(&mut self, dimensions: vek::Extent2<u32>) {
        self.graphics.device().wait();

        // Create a new framebuffer
        let framebuffer =
            Self::create_framebuffer(&self.graphics, dimensions);
