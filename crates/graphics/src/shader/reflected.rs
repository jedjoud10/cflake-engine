...
use naga::{ResourceBinding, TypeInner, AddressSpace};
...
pub fn merge_reflected_module(modules: &[&ReflectedModule]) -> ReflectedShader {
    todo!()
}

// Reflect a naga module's bindings and constants
pub fn reflect_module<M: ShaderModule>(naga: &naga::Module) -> ReflectedModule {
    let groups = reflect_binding_group::<M>(naga);
    ReflectedModule { groups, module: M::stage() }
}

// Fetches the used binding groups of a given naga module
pub fn reflect_binding_group<M: ShaderModule>(naga: &naga::Module) -> Vec<BindGroupLayout> {
    let entries = reflect_binding_entries::<M>(naga);
    let grouped = entries.into_iter().group_by(|x| x.group);
    grouped.into_iter().map(|(index, entries)| {
        BindGroupLayout {
            index,
...
pub fn reflect_binding_entries<M: ShaderModule>(naga: &naga::Module) -> Vec<BindEntryLayout> {
    ...
    vars.iter().filter_map(|(_, value)| {
        ...
        let binding_type = match &type_inner {
            // Uniform Buffers
            TypeInner::Struct { members, span: size } => {
...
fn reflect_buffer(members: &Vec<naga::StructMember>, types: &naga::UniqueArena<naga::Type>, size: &u32, space: AddressSpace) -> BindingType {
        ...
        members: members.iter().map(|member| {
            ...
            let (size, struct_type) = match type_inner {
                ...
                TypeInner::Vector { size, kind, width } => {
                    let size2 = *width as u32 * vector_size_to_u32(size);
                    (size2, StructMemberType::Vector { size: *size, kind: *kind })
                },
                TypeInner::Matrix { columns, rows, width } => {
                    let size = *width as u32 *
                        vector_size_to_u32(columns) *
                        vector_size_to_u32(rows);
                    (size, StructMemberType::Matrix { columns: *columns, rows: *rows })
                },
                _ => panic!()
            };
...
fn naga_stage_to_wgpu_stage(stage: &naga::ShaderStage) -> wgpu::ShaderStages {
    match *stage {
...
fn reflect_texture(class: &naga::ImageClass, dim: &naga::ImageDimension) -> BindingType {
    BindingType::Texture {
