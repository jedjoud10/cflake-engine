...
pub const fn pick_texture_format_channels(
    ...
    match element {
        ElementType::Compressed(compressed) => match (channels, compressed) {
...
pub const fn pick_texture_depth_format(element: ElementType) -> Option<TextureFormat> {
    match element {
        ElementType::Sixteen {
            signed: false,
            normalized: true,
...
pub const fn pick_texture_stencil_format(element: ElementType) -> Option<TextureFormat> {
    match element {
        ElementType::Eight {
            signed: false,
            normalized: false,
...
pub const fn pick_texture_format(
    ...
    match channels {
        TexelChannels::One | TexelChannels::Two | TexelChannels::Four { .. } => {
        ...
        TexelChannels::Srgba { swizzled } => pick_texture_srgb_format(element, swizzled),
    }
}
