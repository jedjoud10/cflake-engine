use vulkan::vk;
use crate::{ChannelsType, BaseType, ColorChannels, ElementType};
...
const fn pick_format_from_color_channels(
    ...
    const fn pick_normalizable_int(bitsize: u32, normalized: bool, signed: bool, channels: ColorChannels) -> vk::Format {
    ...
    const fn pick_non_normalizable_int(bitsize: u32, signed: bool, channels: ColorChannels) -> vk::Format {
        let offset = match bitsize {
    ...
    const fn pick_float(bitsize: u32, channels: ColorChannels) -> vk::Format {
        let offset = match bitsize {
    ...
    match element_type {
        ElementType::Eight { signed, normalized } => pick_normalizable_int(8, normalized, signed, channels),
        ElementType::Sixteen { signed, normalized } => pick_normalizable_int(16, normalized, signed, channels),
        ElementType::ThirtyTwo { signed } => pick_non_normalizable_int(32, signed, channels),
        ElementType::SixtyFour { signed } => pick_non_normalizable_int(64, signed, channels),
