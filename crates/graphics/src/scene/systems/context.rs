...
fn init(world: &mut World, el: &EventLoop<()>, window_settings: WindowSettings, graphic_settings: GraphicsSettings) {
    // Create the winit window
    let raw = crate::context::new_winit_window(el, &window_settings);

    // Create a new Vulkan context
    let graphics = unsafe { crate::context::Graphics::new(&window_settings.title, &raw, graphic_settings) };
    
    // Instantiate a new window wrapper
    let window = unsafe { crate::context::Window::new(el, window_settings, raw, graphics.instance(), graphics.entry()) };
...
pub fn system(system: &mut System, window_settings: WindowSettings, graphic_settings: GraphicsSettings) {
    system
        .insert_init(move |world: &mut World, el: &EventLoop<()>| init(world, el, window_settings, graphic_settings))
        .before(user);
