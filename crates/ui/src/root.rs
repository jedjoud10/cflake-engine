...
#[derive(Debug, Clone)]
pub struct Root {
    pub elements: UnversionnedOrderedVec<Element>,
    pub visible: bool,
    pub max_depth: i32,
    pub root_depth: i32,
}
...
impl Default for Root {
    fn default() -> Self {
        Self {
            elements: UnversionnedOrderedVec::<Element>::default(),
            visible: true,
            max_depth: 0,
            root_depth: 1,
        }
...
impl Root {
    ...
    pub fn add_element(&mut self, mut element: Element) -> usize {
        ...
        let element_id = self.elements.get_next_idx();
    ...
    pub fn get_button_state(&self, element_id: usize) -> &ButtonState {
        // Get the element
        let elem = self.elements.get(element_id).unwrap();
