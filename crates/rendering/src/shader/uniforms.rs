...
// This is a safe wrapper that we can use to modify the uniforms of some shaders
pub struct Uniforms<'a>(&'a mut Program);

impl<'a> Uniforms<'a> {
    // Create a uniforms setter using a shader and a context
    pub fn enable(ctx: &mut Context, shader: impl AsMut<Program> + 'a, closure: impl FnOnce(Uniforms)) {
        unsafe { gl::UseProgram(shader.as_mut().name().get()) }
        closure(Self(shader.as_mut()));
    }
