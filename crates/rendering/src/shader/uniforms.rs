...
// This hints that the underlying program has it's uniforms all set
// This can only be generated by the Populator trait
pub(crate) struct Populated<'p>(Uniforms<'p>);

// A populator is responsible for updating the uniforms of a specific program
// Populators are used internally, and are not shown to the end user, since they are a pain 
pub(crate) unsafe fn populate<'p>(ctx: &mut Context, program: &'p mut Program, populate: impl FnOnce(&mut Uniforms<'p>)) -> Populated<'p> {
    // Bind the program, and set it's uniforms
    ctx.bind(gl::PROGRAM, program.name(), |obj| {
        gl::UseProgram(obj)
    });

    // Update the uniforms (by calling the callback)
    let mut uniforms = Uniforms {
        program,
        bound: 0,
    };
    populate(&mut uniforms);

    // Return the hinted progra
    Populated(uniforms)
}
...
pub struct Uniforms<'uniforms> {
    // The program that will be updated
    program: &'uniforms Program,

    // The count that keeps how many uniforms we bound
    bound: u32,
}
...
impl<'uniforms> Uniforms<'uniforms> {
    ...
    fn set_raw<A: SetRawUniform>(&mut self, name: &'static str, val: A) {
        if let Some(loc) = self.location(name) {
            unsafe { val.set(loc as i32, self.program.name()) }
            self.bound += 1;
        } else {
