...
impl<'canvas, 'context> Rasterizer<'canvas, 'context> {
    ...
    pub(crate) fn new(canvas: &'canvas mut Canvas, ctx: &'context mut Context, settings: RasterSettings) -> Self {
        ...
        unsafe {
            if let Some(mode) = settings.blend {
                gl::Enable(gl::BLEND);
                gl::BlendFunc(transmute::<Factor, u32>(mode.src), transmute::<Factor, u32>(mode.dest));
            } else {
                gl::Disable(gl::BLEND)
            }
        }

        Self { canvas, ctx, settings, primitive }
    }
    ...
    pub unsafe fn draw_from_raw_parts(&mut self, vao: u32, ebo: u32, count: u32, uniforms: &mut Uniforms) -> Result<(), RasterError> {
    ...
    pub fn draw<T: ToRasterBuffers>(&mut self, obj: &T, uniforms: &mut Uniforms) -> Result<(), RasterError> {
        unsafe { self.draw_from_raw_parts(obj.vao().name(), obj.ebo().name(), obj.ebo().len() as u32, uniforms) }
    }
}
