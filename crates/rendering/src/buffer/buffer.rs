...
impl<T: Shared, const TARGET: u32> Buffer<T, TARGET> {
    ...
    pub fn clear_range(&mut self, val: T, range: impl RangeBounds<usize>) -> Result<(), BufferError> {
        todo!()
    }
    ...
    pub fn write_range(&mut self, slice: &[T], range: impl RangeBounds<usize>) -> Result<(), BufferError> {
        todo!()
    }
    ...
    pub fn read_range(&mut self, slice: &mut [T], range: impl RangeBounds<usize>) -> Result<(), BufferError> {
        todo!()
    }

    // Read the whole buffer into a mutable slice
    pub fn read(&mut self, slice: &mut [T]) -> Result<(), BufferError> {
