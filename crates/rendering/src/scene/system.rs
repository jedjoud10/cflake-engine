use super::{
    Camera, ClusteredShading, CommonTextures, Compositor, DirectionalLight, PackedPointLight,
    PointLight, PostProcessing, RenderedFrameStats, Renderer, ShadowMapping,
};
use crate::{
    buffer::BufferMode,
...
fn init(world: &mut World, settings: GraphicsSetupSettings, el: &EventLoop<()>) {
    ...
    let shadow_mapping = ShadowMapping::new(20.0, 160.0, 2048, ctx, &mut shaders, &mut assets);
