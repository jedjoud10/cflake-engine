...
// A simple 2D texture that will contain multiple texels, stored using a specific texel layout
pub struct Texture2D<L: TexelLayout> {
    _phantom: PhantomData<L>,
} 

fn test() {
    let texture = Texture2D::<R<Normalized<i8>>> {
        _phantom: todo!(),
    };
}